...
def process_portfolio(df, prices, is_us=False):
    temp = df.copy()
    if is_us:
        # 미국: Symbol -> 티커, Remarks -> 종목명 으로 변경
    ...
    summary = {'종목명': 'TOTAL', '보유수량': 0, '평균단가': 0, '현재가': 0, '매수금액': total_buy, '평가금액': total_eval, '평가손익': total_eval - total_buy, '수익률(%)': (total_eval/total_buy - 1)*100 if total_buy != 0 else 0}
    
    final_cols = (['티커', '종목명'] if is_us else ['종목명']) + ['보유수량', '평균단가', '매수금액', '현재가', '평가금액', '평가손익', '수익률(%)', '섹터']
    if '계좌명' in temp.columns: final_cols = ['계좌명'] + final_cols
    
    return pd.concat([temp, pd.DataFrame([summary])], ignore_index=True)[final_cols], clean_df
...
with tab_kr:
    ...
    res_kr, clean_kr = process_portfolio(df_kr_raw, kr_prices)
    
    with col1:
        st.subheader("계좌 통합 현황")
        kr_format = {'보유수량':'{:,.0f}','평균단가':'{:,.0f}','매수금액':'{:,.0f}','현재가':'{:,.0f}','평가금액':'{:,.0f}','평가손익':'{:,.0f}','수익률(%)':'{:.2f}%'}
        st.dataframe(res_kr.style.format(kr_format).map(color_profit, subset=['평가손익', '수익률(%)']), use_container_width=True)
    
    with col2:
        st.subheader("섹터별 비중")
        fig_kr = px.pie(clean_kr, values='평가금액', names='섹터', hole=0.4)
...
with tab_us:
    ...
    with col1:
        ...
        us_format = {'보유수량':'{:,.2f}','평균단가':'${:,.2f}','매수금액':'${:,.2f}','현재가':'${:,.2f}','평가금액':'${:,.2f}','평가손익':'${:,.2f}','수익률(%)':'{:.2f}%'}
