...
# 1. 초기 설정
st.set_page_config(page_title="Taeho's Investment OS v8.22", layout="wide")
st.title("🌎 태호님의 글로벌 투자 OS v8.22")
        ...
        for t in ticker_list:
            ...
            if code:
                try:
                    ...
                    results[t] = {"cur": item['nv'], "chg": item['nv'] - item['pcv'], "pct": ((item['nv'] - item['pcv']) / item['pcv'] * 100)}
                except: pass
    else:
        for sym in ticker_list:
            sym = sym.strip().upper()
            if sym in ['CASH', 'NAN', '']: continue
            try:
                # [v8.22] 5일치를 가져와 데이터 결측이 없는 가장 최근 두 날짜를 찾아 계산
                hist = yf.Ticker(sym).history(period="5d")
                if len(hist) >= 2:
                    # 유효한 종가만 필터링하여 마지막 2개 사용
                    valid_closes = hist['Close'].dropna()
                    if len(valid_closes) >= 2:
                        cur_p = float(valid_closes.iloc[-1])
                        prev_p = float(valid_closes.iloc[-2])
                        results[sym] = {"cur": cur_p, "chg": cur_p - prev_p, "pct": ((cur_p - prev_p) / prev_p * 100)}
            except: pass
    return results
...
def build_portfolio(df, prices, account_filter=None, is_us=False):
    ...
    qty_col = 'Qty' if 'Qty' in temp.columns else '보유수량'
    avg_col = 'Price Paid' if 'Price Paid' in temp.columns else '평균단가'
    for _, row in temp.iterrows():
        id_val = str(row.get(id_col, "")).strip().upper()
        name_val = str(row.get('종목명', id_val))
        qty, avg_p = clean_numeric(row.get(qty_col, 0)), clean_numeric(row.get(avg_col, 0))
    ...
    res["비중(%)"] = (res["평가금액"] / res["평가금액"].sum() * 100)
    total = pd.DataFrame([{"종목": "TOTAL", "매수금액": res["매수금액"].sum(), "평가금액": res["평가금액"].sum(), "평가손익": res["평가손익"].sum(), "당일손익": res["당일손익"].sum(), "수익률(%)": (res["평가손익"].sum() / res["매수금액"].sum() * 100) if res["매수금액"].sum() != 0 else 0, "비중(%)": 100.0}])
...
def track_asset_history_hybrid(total_value):
    ...
    except: df_h = pd.read_csv(HISTORY_FILE, parse_dates=['date']) if os.path.exists(HISTORY_FILE) else pd.DataFrame(columns=["date", "total_value"])
    return df_h
...
def display_view(df, title=None, unit="KRW"):
    ...
    fmt = {'보유수량': '{:,.0f}', '비중(%)': '{:.1f}%', '평균단가': '{:,.0f}' if unit=="KRW" else '{:,.2f}', '매수금액': '{:,.0f}' if unit=="KRW" else '{:,.2f}', '현재가': '{:,.0f}' if unit=="KRW" else '{:,.2f}', '평가금액': '{:,.0f}' if unit=="KRW" else '{:,.2f}', '평가손익': '{:,.0f}' if unit=="KRW" else '{:,.2f}', '수익률(%)': '{:.1f}%', '당일변화(%)': '{:.1f}%', '당일손익': '{:,.0f}' if unit=="KRW" else '{:,.2f}'}
    styled_df = df.style.format(fmt, na_rep="-").map(lambda v: f'color: {"#00FF00" if v > 0 else "#FF0000"}' if isinstance(v, (int, float)) and v != 0 else "", subset=['평가손익', '당일손익', '당일변화(%)', '수익률(%)']).apply(lambda x: ['background-color: #222222; font-weight: bold' if x.name == df.index[-1] else '' for _ in x], axis=1)
...
with main_tabs[2]: st.plotly_chart(px.line(history_df, x="date", y="total_value", markers=True), use_container_width=True)